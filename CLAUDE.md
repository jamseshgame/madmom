...
Each stage has multiple interchangeable **engines** under `pipeline/engines/` (e.g. `grid_librosa`/`grid_allinone`/`grid_manual`, `onsets_aubio`/`onsets_basic_pitch`/`onsets_librosa`, `pitches_crepe`/`pitches_yin`/`pitches_basic_pitch`/`pitches_passthrough`). Engine modules side-effect-register themselves into `registry._REGISTRY` at import time via `register_engine`; the router and UI read the catalog through `engines_catalog()`. Stage I/O is validated against pydantic schemas in `pipeline/schemas/`. The `pipeline` router mounts a uniform per-stage GET/POST/DELETE sub-resource (active file + versioned snapshots) from the `_make_stage_subrouter` factory, plus meta endpoints (engine catalog, pipeline state, `run-from`, `build-chart`). The final `build-chart` step serializes to a Clone Hero `notes.chart`. The frontend surfaces this as the editor's **Generate** tab (`frontend/src/components/pipeline/GenerateTab.tsx` + `StageCard.tsx`).
