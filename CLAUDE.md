...
- **Deployment**: DigitalOcean droplet with nginx + systemd. `web/deploy.sh` provisions a *fresh* Ubuntu instance (clones upstream CPJKU — not for redeploys). `web/redeploy.sh` updates the *running* box in place: run it on the droplet (`ssh beatmap 'bash /opt/madmom/web/redeploy.sh'`) and it pulls, rebuilds only what changed (pip/npm/Cython), restarts the `beatmap-backend` service when backend code or deps moved, and health-checks. It aborts if a dep install would reinstall/downgrade torch/torchaudio (the torchcodec-breaking trap; `--allow-torch` overrides). nginx config in `web/nginx/`, systemd unit in `web/systemd/`. The droplet is the SSH host `beatmap` (root); frontend is the static Vite build served from `web/frontend/dist`.
...
**torch + torchaudio are pinned to the 2.12/2.11 line in `constraints.txt`** (`torch==2.12.*`, not an exact patch — the dev box runs 2.12.0, the droplet 2.12.1, both fine; an exact pin forces a torchcodec-breaking downgrade on the droplet). A minor bump is what actually breaks `torchcodec` (the path `torchaudio.save()` takes), so `.*` blocks that while allowing the patch. **torchvision is NOT in the main requirements or constraints** — it hard-pins a torch patch, so `onnx2torch`/`torchvision` (needed only by audio-separator's MDX-Net models) live in `requirements-extras.txt` installed `--no-deps`, dropping the wheels in against whatever torch is present. Roformer/MDXC and VR Arch — the default hybrid engine and every top vocal model — need none of it, so MDX-Net is an optional bonus that never risks the torch install.
...
### Draft tracks (resumable imports)

A `Track` (`app/services/tracks.py`) has a `status` field: `draft` (master audio staged, separation not yet run) or `ready` (stems exist — the normal state). Missing = `ready`, so pre-existing `track.json` files need no migration. `create_draft_track` persists the untouched master under `<track>/source/` (separate from `stems/` so publish/zip helpers never pick it up) and lists the track immediately; `promote_draft` flips it to `ready` in place once stems exist, keeping the same id so links stay valid. The master is kept after separation, so a track can be re-split with a different engine without re-uploading.

The Create page (`CreateSection.tsx`) POSTs staged audio to `POST /api/tracks/draft` the moment it's staged (YouTube pull or dropped file), *before* the settings screen — previously a multi-minute YouTube download lived only as a browser `File` and was lost if the tab was abandoned. Drafts show in the Studio Library with an "In progress" badge and a Resume button that deep-links to `/?draft=<id>`; the Create page hydrates from the draft and, on Generate, calls `POST /api/tracks/{id}/separate` (stored master → `separate_with_engine` → promote) rather than the upload-based `/api/stems/separate`. `GET /api/tracks/{id}/source` streams the stored master back.
